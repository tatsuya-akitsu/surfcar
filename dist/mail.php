...
$thanksPage = "http://stg.powder.bz/surfcar/thanks.html";
...
$refrom_name = "株式会社SurfCar";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "SurfCarへお申込みありがとうございました";
...
この度はSurfCarSystemにお申込みをいただき、誠にありがとうございます。
...
URL: https://peraichi.com/landing_pages/view/surfcar#section-7
──────────────────────
...
<section class="section confirm">
	<?php if($empty_flag == 1){ ?>
	<div class="wrapper">
		<h2 class="section__title--simple">入力にエラーがあります。<br >下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h2>
		<div class="inner">
			<?php echo $errm; ?>
			<button class="c-button c-button--back" onClick="history.back()">前画面に戻る</button>
		</div>
	</div>
	<?php }else{ ?>
	<div class="wrapper">
		<h2 class="section__title--simple">確認画面</h2>
		<div class="inner">
			<p class="confirm-text">以下の内容で間違いがなければ、送信するボタンを押してください</p>
			<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="post">
				<?php echo confirmOutput($_POST);//入力内容を表示?>
				<p align="center"><input type="hidden" name="mail_set" value="confirm_submit">
				<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']);?>">
				<button type="button" class="c-button c-button--back" onClick="history.back()">前画面に戻る</button>
				<button type="send" class="c-button c-button--send">送信する</button>
			</form>
		</div>
	</div>
	<?php } ?>
...
<body>
	<?php if($empty_flag == 1){ ?>
		<section class="section error">
			...
			<div class="inner">
				<div class="error-wrap"><?php echo $errm; ?></div>
				<button type="button" class="c-button c-button--back" onClick="history.back()">前画面に戻る</button>
			</div>
		</section>
	</body>
	...
	<?php }else{ ?>
  <section id="thanks" class="section thanks js-thanks">
    <h1>SurfCarSystem</h1>
    <div class="video-wrap">
      <video muted loop autoplay playsinline class="is-pc">
        <source src="./img/video/driving.mp4" />
        <source src="./img/video//driving.webm" />
      </video>
      <img src="./img/video/video--sp.jpg" alt="" class="is-sp">
    </div>
    <div class="wrapper">
      <h2 class="section__title--simple">お問い合わせありがとうございます</h2>
      <div class="inner">
        <p class="point__intro">
          この度はSurfCarに<br class="is-sp" >お問い合わせありがとうございました。<br>
          ご入力いただきました、メールアドレスに<br class="is-sp" >自動返信メールをお送りしております。<br>
          担当者からのご連絡を今しばらくお待ち下さい。
        </p>
        <a href="http://stg.powder.bz/surfcar/" class="c-button c-button--home">トップへ戻る</a>
      </div>
    </div>
  </section>
...
function confirmOutput($arr){
	...
	foreach($arr as $key => $val) {
		...
		$html .= "<div class='confirm-item'><p class='confirm-label'>".$key."</p><p class='confirm-value'>".$out;
